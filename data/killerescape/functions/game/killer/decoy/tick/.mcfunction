...
#被弾
execute positioned ~ ~-1 ~ as @a[tag=survivor,tag=!eliminated,distance=..1,limit=1,sort=nearest] at @s run function killerescape:game/killer/decoy/launch/___
execute positioned ~ ~-1 ~ if entity @a[tag=survivor,tag=!eliminated,distance=..1] run scoreboard players set @s time 200
