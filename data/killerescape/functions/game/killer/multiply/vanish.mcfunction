...
#タマシイ持ちなら回収
execute if entity @s[tag=hassoul] run loot give @a[tag=killer] loot killerescape:soul
execute if entity @s[tag=hassoul] run tellraw @a[tag=killer] {"text":"[システム] タマシイを回収しました","color": "yellow"}
...
tp @s ~ ~-1000 ~
kill @s[type=minecraft:interaction,tag=current]
