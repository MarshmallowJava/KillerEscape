...
execute if score @s vampire_count < remain var run title @s actionbar {"translate":"%s (%s/%s個所持 獲得まで%s)","with":[{"entity":"@s","nbt":"SelectedItem.tag.display.Lore[0]","interpret":true},{"score":{"name":"@s","objective":"vampire_count"}},{"score":{"name":"remain","objective":"var"}},{"storage":"minecraft:killerescape","nbt":"result","interpret":true}]}
