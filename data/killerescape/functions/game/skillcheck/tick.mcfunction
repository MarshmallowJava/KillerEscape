...
#表示
execute if score @s skillcheck_time matches 40.. run title @s[scores={skillcheck_ans=0}] actionbar {"translate":"[%sキーを押さないでください！！！]","italic": false,"color": "yellow","with":[{"keybind":"key.swapOffhand"}]}
execute if score @s skillcheck_time matches 40.. run title @s[scores={skillcheck_ans=1}] actionbar {"translate":"[%sキーを押してください！！！]","italic": false,"color": "red","with":[{"keybind":"key.swapOffhand"}]}
