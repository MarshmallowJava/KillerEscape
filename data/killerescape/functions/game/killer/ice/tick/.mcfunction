...
scoreboard players add @s[scores={cool_ice=..-2}] cool_ice 1

#クールダウン設定
execute if score @s ice_time matches ..0 run function killerescape:game/killer/ice/stop
execute if entity @s[nbt={SelectedItem:{tag:{CustomTag:Ice}}},scores={useItem=1,cool_ice=-1}] run function killerescape:game/killer/ice/stop
...
title @s[tag=display_ui,scores={cool_ice=..-2}] actionbar {"translate":"効果発動中 視界内の生存者の体温を低下させています(残り冷気 %s)","with":[{"storage":"minecraft:killerescape","nbt":"result","interpret":true}]}
title @s[tag=display_ui,scores={cool_ice=-1}] actionbar {"translate":"能力アイテム右クリックで中断する (残り冷気 %s)","with":[{"storage":"minecraft:killerescape","nbt":"result","interpret":true}]}
