...
execute if entity @s[tag=display_ui,nbt={SelectedItem:{tag:{CustomTag:KillerWeapon}}}] run title @s actionbar {"translate":"%s%s","with":[{"entity":"@s","nbt":"SelectedItem.tag.display.Lore[0]","interpret":true},{"text":"+回復不可付与可能","color": "light_purple"}]}
